...
class PlayerInfoReq :IPacket
{
	...
	public class Skill
	{
		...
		public struct Attribute
		{
		    ...
		    public Attribute Read(ReadOnlySpan<byte> s, ref ushort count)
		    {
	    ...
	    public Skill Read(ReadOnlySpan<byte> s, ref ushort count)
	    {
			...
			ushort attributeLen = (ushort)BitConverter.ToInt16(s.Slice(count, s.Length - count));
			count += sizeof(ushort);
			
			for (int i =0; i < attributeLen; i++)
			{
			    attributes.Add(new Attribute().Read(s, ref count));
			}
	        return this;
	    }
	    ...
	    public bool Write(Span<byte> s , ref ushort count)
	    {
			...
			count += sizeof(float);
			ushort attributeLne = (ushort)attributes.Count;
			success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), attributeLne);
			count += sizeof(ushort);
			for (int i = 0; i < attributes.Count; i++)
			{
			    attributes[i].Write(s,ref count);
			}
	        return success;
	    }
	}
    ...
    public void Read(ArraySegment<byte> segment)
    {
		...
		{
		    skills.Add(new Skill().Read(s, ref count));
		}
    }
